...
namespace Shop.Web.Areas.Admin.Controllers
{
    public class ProductController : AdminBaseController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                switch (result)
                {
                    case CreateProductResult.NotImage:
                        TempData[WarningMessage] = "لصفا برای محصول یک تصویر انتخاب کنید";
                        break;
        ...
        {
            var data = _productService.GetEditProduct(productId);
            if (data == null)
            {
                return NotFound();
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                switch (result)
                {
                    ...
                    case EditProductResult.NotProductSelectedCategoryHasNull:
                        TempData[WarningMessage] = "لصفا دسته بندی محصول را وارد کنید";
                        break;
        ...
        [HttpPost, ValidateAntiForgeryToken]
        public async Task<IActionResult> CreateProductCategory(CreateProductCategoryViewModel createProductCategory, Microsoft.AspNetCore.Http.IFormFile image)
        {
            if (ModelState.IsValid)
            {
        ...
        {
            var mdel = new CreateProductFeatuersViewModel
            {
                productId = productId,
            };
            ...
            if (ModelState.IsValid)
            {
                ...
                switch (result)
                {
                    case CreateProductFeatuersResult.Error:
                        TempData[WarningMessage] = "در ثبت ویژگی خطایی رخ داده است";
                        break;
